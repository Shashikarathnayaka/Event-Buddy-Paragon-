...
// ✅ FIX: ConsumerWidget වෙනුවට ConsumerStatefulWidget use කරන්න
...
class _NavigationScreenState extends ConsumerState<NavigationScreen> {
  ...
  // ✅ FIX: Firebase එකෙන් organizer status check කරන්න
  Future<void> _checkOrganizerStatus() async {
    final user = FirebaseAuth.instance.currentUser;

    print('🔍 NavigationScreen - Checking organizer status');
    print('   User ID: ${user?.uid}');
    print('   Widget isOrganizer: ${widget.isOrganizer}');
    ...
    try {
      ...
      print('✅ Organizer status checked:');
      print('   Document exists: ${orgDoc.exists}');
      print('   Is Organizer: $_isOrganizer');

      if (orgDoc.exists) {
        print('   Organizer data: ${orgDoc.data()}');
      }
    } catch (e) {
      print('❌ Error checking organizer status: $e');
      setState(() {
        // Fallback to widget value if error
        _isOrganizer = widget.isOrganizer ?? false;
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    print('🏗️ Building NavigationScreen:');
    print('   Selected Index: $selectedIndex');
    print('   Is Organizer: $_isOrganizer');

    // ✅ FIX: ?? true remove කළා - දැන් හරි value එක use කරනවා
    final pages = [
      HomeScreen(isOrganizer: _isOrganizer),
      MyEventsContent(isOrganizer: _isOrganizer),
      const ProfileScreen(),
    ];
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      close(context, null);
                      // ✅ FIX: Proper route with ID
                      context.push(
                        '/event/${event.id}',
                        extra: {'event': event, 'isOrganizer': isOrganizer},
                      );
                    },
                  ),
                );
